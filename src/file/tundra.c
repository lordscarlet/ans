...
uint32_t TUNDRA_IMAGE_CHUNK_LENGTH  = 10000 * 80 * 7;
...
void* realloc_tundra_image_bytes_if_necessary(uint8_t *image_bytes, uint32_t *limit, uint32_t pos)
{
    void* alloc = image_bytes;
    if(pos >= *limit)
    {
        while(*limit < pos) {
            *limit += TUNDRA_IMAGE_CHUNK_LENGTH;
        }
...
TundraFile* load_tundra(char const *filename)
{
    ...
    uint32_t   max_y = 0;
    uint32_t   image_bytes_limit = TUNDRA_IMAGE_CHUNK_LENGTH;
    ...
    for(uint32_t i = 0, x = 0, y = 0, image_bytes_location; i < file->actual_file_size;)
    {
        if(x == 80)
        {
            x = 0;
        ...
        if (ascii_code != 1 && ascii_code != 2 && ascii_code != 4 && ascii_code != 6) {
            image_bytes_location = (y * 80 + x) * 7;
            file->image_bytes = realloc_tundra_image_bytes_if_necessary(file->image_bytes, &image_bytes_limit, image_bytes_location);
    ...
    file->rows = max_y;
    file->image_bytes = realloc(file->image_bytes, 80 * max_y * 7);
    fclose(file_ptr);
    free(foreground);
    free(background);
    return file;
...
{
    Canvas *canvas = create_canvas(640, file->rows * TUNDRA_DEFAULT_FONT_HEIGHT);
