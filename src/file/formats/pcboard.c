...
uint32_t PCBOARD_IMAGE_CHUNK_LENGTH  = 10000 * 80 * 7;
...
void* realloc_pc_board_image_bytes_if_necessary(uint8_t *image_bytes, uint32_t *limit, uint32_t pos)
{
    void* alloc = image_bytes;
    if(pos >= *limit)
    {
        while(*limit < pos) {
            *limit += PCBOARD_IMAGE_CHUNK_LENGTH;
        }
        alloc = calloc(*limit, 1);
        memcpy(alloc, image_bytes, *limit);
        free(image_bytes);
    }
    return alloc;
}
...
PCBoardFile* load_pcboard(char const *filename)
{
    ...
    uint32_t image_bytes_limit = PCBOARD_IMAGE_CHUNK_LENGTH;
...
LITERAL:
            ...
            image_bytes_location = (y * 80 + x) * 2;
            file->image_bytes = realloc_pc_board_image_bytes_if_necessary(file->image_bytes, &image_bytes_limit, image_bytes_location);
    ...
    file->image_bytes = realloc(file->image_bytes, 80 * file->rows * 2);
    return file;
}
...
{
    Canvas *canvas = create_canvas(640, file->rows * PCBOARD_DEFAULT_FONT_HEIGHT);
    ...
    for(size_t y = 0, i = 0; y < file->rows; y += 1)
    {
        for(size_t x = 0; x < 80; x += 1, i += 2)
        {
            ...
            draw_glyph(canvas, ascii_code, foreground, background, x, y, palette_rgb, font_bits, 8, PCBOARD_DEFAULT_FONT_HEIGHT);
        }
    }
